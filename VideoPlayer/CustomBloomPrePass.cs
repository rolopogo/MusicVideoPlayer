...
namespace MusicVideoPlayer
{
    class CustomBloomPrePass : MonoBehaviour, CameraRenderCallbacksManager.ICameraRenderCallbacks
    {
        ...
        KawaseBlurRendererSO _kawaseBlurRenderer;
        ...
        private void Start()
        {
            ...
            BloomPrePassRendererSO bppr = Resources.FindObjectsOfTypeAll<BloomPrePassRendererSO>().First();
            _kawaseBlurRenderer = bppr.GetPrivateField<KawaseBlurRendererSO>("_kawaseBlurRenderer");
        ...
        public void OnCameraPreRender(Camera camera)
        {
            ...
            DoubleBlur(temporary, blur2, KawaseBlurRendererSO.KernelSize.Kernel127, 0.07f, KawaseBlurRendererSO.KernelSize.Kernel15, 0.03f, bloomPrePassParams.bloom2Alpha, bloomPrePassParams.downsample);
        ...
        private void DoubleBlur(RenderTexture src, RenderTexture dest, KawaseBlurRendererSO.KernelSize kernelSize0, float boost0, KawaseBlurRendererSO.KernelSize kernelSize1, float boost1, float secondBlurAlpha, int downsample)
        {
            ...
            _kawaseBlurRenderer.Blur(src, temporary, blurKernel, 0f, downsample, 0, num, 0f, 1f, false, false, KawaseBlurRendererSO.WeightsType.None);
            _kawaseBlurRenderer.Blur(temporary, dest, blurKernel, boost0, 0, num, blurKernel.Length - num, 0f, 1f, false, false, KawaseBlurRendererSO.WeightsType.None);
            _kawaseBlurRenderer.Blur(temporary, dest, blurKernel2, boost1, 0, num, blurKernel2.Length - num, 0f, secondBlurAlpha, true, false, KawaseBlurRendererSO.WeightsType.None);
            RenderTexture.ReleaseTemporary(temporary);
        }
    }
}
